...
const EditProfile = () => {
    ...
    const handleSubmit = async (values : EmployeeDataFormat, { setSubmitting, setErrors,setValues }:FormikHelpers<EmployeeDataFormat>) => {
        try {
            const result = await appClient.patch(`api/employees/${userId}/`, values);
    ...
    const handleDocumentsSubmit = async (values : DocumentsFormatTypes, { setSubmitting, setErrors ,setValues} : FormikHelpers<DocumentsFormatTypes>) => {
        ...
        try {
            ...
            if (!employeeDocuments) {
                response = await appClient.post(`/employeeDocuments/`, formData, {
                    headers: { 'Content-Type': 'multipart/form-data' },
                });
            } else {
                response = await appClient.patch(`/employeeDocuments/${employeeDocuments.id}/`, formData, {
                    headers: { 'Content-Type': 'multipart/form-data' },
                });
            }
